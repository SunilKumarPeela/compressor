...
constexpr int ID_MENU_COMPRESS_FILE = 241;
constexpr int ID_MENU_DECOMPRESS_FILE = 242;
constexpr int ID_MENU_COMPRESS_FILE_BALANCED = 243;
...
constexpr UINT_PTR INBOX_EVENT_POLL_TIMER_ID = 4;
...
constexpr UINT WM_POINT_COMPRESSION_COMPLETE = WM_APP + 23;
HANDLE inbox_changed_event = nullptr;
...
std::filesystem::path application_install_root;
...
std::atomic_bool compression_running{false};

struct CompressionCompletion {
    bool decompress = false;
    point::compression::Profile profile = point::compression::Profile::Fast;
    bool succeeded = false;
    std::filesystem::path output;
    point::compression::FileResult result;
    std::wstring error;
};
...
HMODULE risk_rich_edit_module = nullptr;
bool risk_report_supports_formatting = false;
...
struct CellUndoValue {
    int row = 0;
    int column = 0;
    std::string before;
    std::string after;
};
std::deque<std::vector<CellUndoValue>> cell_undo_history;
std::size_t cell_undo_history_bytes = 0;
bool applying_undo_history = false;
std::optional<std::uint64_t> focused_edit_key;
std::string focused_edit_original;

struct WorkspaceOperationUndo {
    enum class Kind { TextTransformation, DuplicateRemoval };
    Kind kind = Kind::TextTransformation;
    int column = -1;
    std::vector<std::pair<int, std::string>> text_values;
    DuplicateRemovalUndo duplicate;
};
std::deque<WorkspaceOperationUndo> workspace_operation_history;
std::size_t workspace_operation_history_bytes = 0;
...
void record_workspace_text_undo(
    int column, const std::vector<std::pair<int, std::string>>& values);
void record_duplicate_removal_undo(const DuplicateRemovalUndo& undo);
void undo_workspace_operation();
...
bool column_accepts_identity_name_resolution(int column) {
    if (column < 0 || column >= GRID_COLUMNS) return false;
    auto field = point::normalize_name(point::trim(narrow(control_text(
        header_cells[static_cast<std::size_t>(column)]))));
    for (const auto& group : field_synonym_groups) {
        const auto canonical =
            point::normalize_name(group.canonical_field);
        if (field == canonical || std::any_of(
                group.synonyms.begin(), group.synonyms.end(),
                [&](const std::string& synonym) {
                    return point::normalize_name(synonym) == field;
                })) {
            field = canonical;
            break;
        }
    }
    static const std::unordered_set<std::string> identity_targets = {
        "employeeid", "username", "samaccountname", "email", "userid",
        "userprincipalname"
    };
    return identity_targets.contains(field);
}
...
void record_cell_undo_action(std::vector<CellUndoValue> action) {
    if (applying_undo_history) return;
    action.erase(std::remove_if(action.begin(), action.end(),
        [](const CellUndoValue& value) {
            return value.before == value.after;
        }), action.end());
    if (action.empty()) return;
    std::size_t bytes = action.size() * sizeof(CellUndoValue);
    for (const auto& value : action)
        bytes += value.before.size() + value.after.size();
    constexpr std::size_t MAX_HISTORY_BYTES = 64ull * 1024ull * 1024ull;
    if (bytes > MAX_HISTORY_BYTES) {
        cell_undo_history.clear();
        cell_undo_history_bytes = 0;
        return;
    }
    while (!cell_undo_history.empty() &&
           (cell_undo_history.size() >= 100 ||
            cell_undo_history_bytes + bytes > MAX_HISTORY_BYTES)) {
        auto& oldest = cell_undo_history.front();
        cell_undo_history_bytes -= oldest.size() * sizeof(CellUndoValue);
        for (const auto& value : oldest)
            cell_undo_history_bytes -=
                value.before.size() + value.after.size();
        cell_undo_history.pop_front();
    }
    cell_undo_history_bytes += bytes;
    cell_undo_history.push_back(std::move(action));
}

void undo_cell_action() {
    if (cell_undo_history.empty())
        throw std::runtime_error("There are no more cell edits to undo");
    auto action = std::move(cell_undo_history.back());
    cell_undo_history.pop_back();
    std::size_t bytes = action.size() * sizeof(CellUndoValue);
    for (const auto& value : action)
        bytes += value.before.size() + value.after.size();
    cell_undo_history_bytes = bytes > cell_undo_history_bytes
        ? 0 : cell_undo_history_bytes - bytes;
    applying_undo_history = true;
    for (const auto& value : action) {
        store_cell(value.row, value.column, value.before);
        pending_identity_resolution_cells.insert(
            cell_key(value.row, value.column));
        if (universal_mode_active() && universal_results_displayed &&
            value.column == 0)
            universal_pending_lookup_rows.insert(value.row);
    }
    applying_undo_history = false;
    load_visible_cells();
    point::append_audit(app_root, "CELL_EDIT_UNDO",
        "cells=" + std::to_string(action.size()));
    set_status(L"Undid one cell-edit action. " +
        std::to_wstring(cell_undo_history.size()) +
        L" earlier action(s) remain.");
}

void finalize_focused_edit_history() {
    if (!focused_edit_key || applying_undo_history) return;
    const auto key = *focused_edit_key;
    const int row = static_cast<int>(
        key / static_cast<std::uint64_t>(GRID_COLUMNS));
    const int column = static_cast<int>(
        key % static_cast<std::uint64_t>(GRID_COLUMNS));
    record_cell_undo_action({
        {row, column, focused_edit_original, stored_cell(row, column)}});
    focused_edit_key.reset();
    focused_edit_original.clear();
}
...
void apply_quick_text_transform(
    ...
    record_workspace_text_undo(
        position.column, {{position.row, before}});
...
void apply_learned_transformation(bool selected_rows_only) {
    ...
    std::vector<std::pair<int, std::string>> action_values;
    action_values.reserve(preview.matching_rows.size());
    for (const int row : preview.matching_rows) {
        ...
        if (!after) continue;
        duplicate_removal_undo = {};
        action_values.push_back({row, before});
        store_cell(row, learned_text_transformation.column, *after);
    }
    record_workspace_text_undo(
        learned_text_transformation.column, action_values);
    load_visible_cells();
    set_status(
...
void remove_duplicate_workspace_rows(int key_column) {
    ...
    record_duplicate_removal_undo(duplicate_removal_undo);
...
std::size_t workspace_operation_size(const WorkspaceOperationUndo& action) {
    std::size_t bytes = sizeof(action);
    for (const auto& [row, value] : action.text_values) {
        (void)row;
        bytes += sizeof(row) + value.size();
    }
    for (const auto& removed : action.duplicate.removed_rows) {
        bytes += sizeof(removed);
        for (const auto& [column, value] : removed.values) {
            (void)column;
            bytes += sizeof(column) + value.size();
        }
    }
    return bytes;
}

void push_workspace_operation(WorkspaceOperationUndo action) {
    constexpr std::size_t MAX_BYTES = 64ull * 1024ull * 1024ull;
    const auto bytes = workspace_operation_size(action);
    if (bytes > MAX_BYTES) return;
    while (!workspace_operation_history.empty() &&
           (workspace_operation_history.size() >= 20 ||
            workspace_operation_history_bytes + bytes > MAX_BYTES)) {
        workspace_operation_history_bytes -= workspace_operation_size(
            workspace_operation_history.front());
        workspace_operation_history.pop_front();
    }
    workspace_operation_history_bytes += bytes;
    workspace_operation_history.push_back(std::move(action));
}

void record_workspace_text_undo(
        int column,
        const std::vector<std::pair<int, std::string>>& values) {
    if (applying_undo_history || values.empty()) return;
    WorkspaceOperationUndo action;
    action.kind = WorkspaceOperationUndo::Kind::TextTransformation;
    action.column = column;
    action.text_values = values;
    push_workspace_operation(std::move(action));
}

void record_duplicate_removal_undo(const DuplicateRemovalUndo& undo) {
    if (applying_undo_history || !undo.available) return;
    WorkspaceOperationUndo action;
    action.kind = WorkspaceOperationUndo::Kind::DuplicateRemoval;
    action.duplicate = undo;
    push_workspace_operation(std::move(action));
}

void undo_workspace_operation() {
    if (workspace_operation_history.empty())
        throw std::runtime_error(
            "There are no more transformations to undo");
    const auto action = workspace_operation_history.back();
    applying_undo_history = true;
    try {
        if (action.kind ==
            WorkspaceOperationUndo::Kind::TextTransformation) {
            for (const auto& [row, value] : action.text_values)
                store_cell(row, action.column, value);
            load_visible_cells();
            point::append_audit(app_root, "TEXT_TRANSFORM_UNDO",
                "restored=" + std::to_string(action.text_values.size()));
        } else {
            duplicate_removal_undo = action.duplicate;
    ...
    applying_undo_history = false;
    workspace_operation_history_bytes -= workspace_operation_size(
        workspace_operation_history.back());
    workspace_operation_history.pop_back();
    set_status(L"Undid one workspace operation. " +
        std::to_wstring(workspace_operation_history.size()) +
        L" earlier operation(s) remain.");
}
...
LRESULT CALLBACK grid_edit_subclass(
    ...
    case WM_CONTEXTMENU: {
        ...
        if (is_header_id(GetDlgCtrlID(control))) {
            ...
            AppendMenuW(
                menu,
                workspace_operation_history.empty()
                    ? MF_GRAYED : MF_STRING,
                MENU_UNDO_TRANSFORM,
                L"Undo Workspace Operation\tCtrl+U");
        }
        if (!is_header_id(GetDlgCtrlID(control))) {
            AppendMenuW(menu, MF_SEPARATOR, 0, nullptr);
            ...
            AppendMenuW(menu, MF_STRING, MENU_REMOVE_DUPLICATE_ROWS,
                L"Remove Exact Duplicate Rows...");
            AppendMenuW(menu,
                workspace_operation_history.empty() ? MF_GRAYED : MF_STRING,
                MENU_UNDO_DUPLICATE_REMOVAL,
                L"Undo Last Workspace Operation\tCtrl+U");
        }
        POINT pointer{
            GET_X_LPARAM(lparam),
            GET_Y_LPARAM(lparam)};
...
void clear_grid() {
    ...
    cell_undo_history.clear();
    cell_undo_history_bytes = 0;
    workspace_operation_history.clear();
    workspace_operation_history_bytes = 0;
    focused_edit_key.reset();
    internal_cell_update = true;
        ...
        try {
            ...
            completion->refreshed->load_files_incremental(
                ...
                [&](std::size_t current, std::size_t total,
                    ...
                    post_progress(
                        percent,
                        std::wstring(reused ? L"Reusing loaded report: "
                                            : L"Loading changed report: ") +
                            file.filename().wstring());
                }, [] { return refresh_cancel_requested.load(); });
...
void add_auto_import_schedules() {
    ...
    const COMDLG_FILTERSPEC filters[] = {
        {L"CSV and Excel files", L"*.csv;*.xlsx;*.xls"},
        {L"All files", L"*.*"}};
...
void import_input_files(const std::vector<std::filesystem::path>& files) {
    ...
    if (imported) {
        ...
        MessageBoxW(main_window,
            L"No files were added. Point accepts CSV, XLSX, and XLS. Macro-enabled XLSM is blocked.",
...
void choose_input_files() {
    ...
    dialog.lpstrFilter = L"Supported data files (*.csv;*.xlsx;*.xls)\0"
        L"*.csv;*.xlsx;*.xls\0All files (*.*)\0*.*\0\0";
...
void clear_selected_range() {
    ...
    std::vector<CellUndoValue> clear_undo;
    for (const auto& [key, value] : cell_store) {
        const int row = static_cast<int>(
            key / static_cast<std::uint64_t>(GRID_COLUMNS));
        const int column = static_cast<int>(
            key % static_cast<std::uint64_t>(GRID_COLUMNS));
        if (row >= std::max(0, first_row) && row <= last_row &&
            column >= first_column && column <= last_column)
            clear_undo.push_back({row, column, value, {}});
    }
    internal_cell_update = true;
    if (first_row <= -1) {
        for (int column = first_column;
    ...
    invalidate_cell_row_indexes();
    record_cell_undo_action(std::move(clear_undo));
    focused_edit_key.reset();
    focused_edit_original.clear();
    internal_cell_update = false;
    load_visible_cells();
...
std::optional<std::filesystem::path> choose_compression_input(
        bool decompress) {
    std::vector<wchar_t> buffer(65'536, L'\0');
    OPENFILENAMEW dialog{};
    dialog.lStructSize = sizeof(dialog);
    dialog.hwndOwner = main_window;
    dialog.lpstrFilter = decompress
        ? L"Point compressed files (*.pfc)\0*.pfc\0All files (*.*)\0*.*\0\0"
        : L"All files (*.*)\0*.*\0\0";
    dialog.lpstrFile = buffer.data();
    dialog.nMaxFile = static_cast<DWORD>(buffer.size());
    dialog.Flags = OFN_EXPLORER | OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST |
        OFN_DONTADDTORECENT;
    dialog.lpstrTitle = decompress
        ? L"Select a Point file to decompress"
        : L"Select a file to compress with Point";
    if (!GetOpenFileNameW(&dialog)) return std::nullopt;
    return std::filesystem::path(buffer.data());
}

std::optional<std::filesystem::path> choose_compression_output(
        const std::filesystem::path& input, bool decompress) {
    std::filesystem::path suggested;
    if (decompress) {
        suggested = input;
        if (_wcsicmp(suggested.extension().c_str(), L".pfc") == 0)
            suggested.replace_extension();
        else
            suggested += L".decompressed";
        if (suggested == input) suggested += L".decompressed";
    } else {
        suggested = input;
        suggested += L".pfc";
    }
    std::vector<wchar_t> buffer(65'536, L'\0');
    const auto suggested_text = suggested.wstring();
    if (suggested_text.size() + 1 > buffer.size())
        throw std::runtime_error("Suggested output path is too long");
    ...
    dialog.Flags = OFN_EXPLORER | OFN_PATHMUSTEXIST | OFN_OVERWRITEPROMPT |
        OFN_DONTADDTORECENT;
    dialog.lpstrDefExt = decompress ? nullptr : L"pfc";
    dialog.lpstrTitle = decompress
        ? L"Save decompressed file as"
        : L"Save Point compressed file as";
    if (!GetSaveFileNameW(&dialog)) return std::nullopt;
    return std::filesystem::path(buffer.data());
}

void start_file_compression(
        bool decompress,
        point::compression::Profile profile = point::compression::Profile::Fast) {
    if (compression_running.load()) {
        MessageBoxW(main_window,
            L"A compression or decompression job is already running.",
            L"Point Fast Compression", MB_ICONINFORMATION);
        return;
    }
    try {
        const auto input = choose_compression_input(decompress);
        if (!input) return;
        const auto output = choose_compression_output(*input, decompress);
        if (!output) return;
        ...
        if (_wcsicmp(input_absolute.c_str(), output_absolute.c_str()) == 0)
            throw std::runtime_error("Input and output must be different files");
        if (compression_thread.joinable()) compression_thread.join();
        compression_running.store(true);
        const bool balanced = profile == point::compression::Profile::Balanced;
        set_status(decompress
            ? L"Point is decompressing the selected file..."
            : balanced
                ? L"Point Balanced Mode is compressing the selected file..."
                : L"Point Fast Mode is compressing the selected file...");
        compression_thread = std::thread(
            [input_absolute, output_absolute, decompress, profile]() {
                auto completion = std::make_unique<CompressionCompletion>();
                completion->decompress = decompress;
                completion->profile = profile;
                completion->output = output_absolute;
                try {
                    completion->result = decompress
                        ? point::compression::decompress_file_fast(
                            input_absolute, output_absolute)
                        : point::compression::compress_file(
                            input_absolute, output_absolute, profile);
                    completion->succeeded = true;
                } catch (const std::exception& ex) {
                    completion->error = widen(ex.what());
                }
                if (main_window && IsWindow(main_window) &&
                    PostMessageW(main_window, WM_POINT_COMPRESSION_COMPLETE, 0,
                        reinterpret_cast<LPARAM>(completion.get())))
                    completion.release();
            });
    } catch (const std::exception& ex) {
        compression_running.store(false);
        MessageBoxW(main_window, widen(ex.what()).c_str(),
            L"Point Fast Compression", MB_ICONERROR);
    }
}
...
void paste_at_cell(HWND focus) {
    ...
    internal_cell_update = true;
    std::size_t queued_for_resolution = 0;
    if (is_header_id(id)) {
        if (!pasted.empty()) {
            for (std::size_t column = 0;
        ...
        std::vector<CellUndoValue> paste_undo;
        std::size_t pasted_cell_count = 0;
        for (const auto& row : pasted) pasted_cell_count += row.size();
        paste_undo.reserve(pasted_cell_count);
        for (std::size_t row = 0;
             row < pasted.size() &&
            ...
            for (std::size_t column = 0;
                 ...
                 start_column + static_cast<int>(column) < GRID_COLUMNS;
                 ++column) {
                const int logical_row = start_row + static_cast<int>(row);
                const int logical_column =
                    start_column + static_cast<int>(column);
                const auto after = narrow(pasted[row][column]);
                paste_undo.push_back({logical_row, logical_column,
                    stored_cell(logical_row, logical_column), after});
                store_cell(logical_row, logical_column, after);
                if (!pasted[row][column].empty() &&
                    column_accepts_identity_name_resolution(logical_column)) {
                    pending_identity_resolution_cells.insert(cell_key(
                        start_row + static_cast<int>(row), logical_column));
                    ++queued_for_resolution;
                }
            }
        }
        record_cell_undo_action(std::move(paste_undo));
        focused_edit_key.reset();
        focused_edit_original.clear();
        load_visible_cells();
        if (queued_for_resolution) {
            point::append_audit(
                app_root, "PASTE_IDENTITY_RESOLUTION_QUEUED",
                std::to_string(queued_for_resolution) +
                " identity cell(s) queued; values not recorded");
        }
    }
    internal_cell_update = false;
    hide_suggestions();
    set_status(queued_for_resolution
        ? L"Clipboard pasted. Identity values will be validated when Search is clicked."
...
bool handle_keyboard_shortcut(const MSG& message) {
    ...
    if (message.wParam == 'Z') {
        try {
            if (focused_edit_key) {
                commit_grid_control(focus);
                finalize_focused_edit_history();
            }
            undo_cell_action();
        } catch (const std::exception& ex) {
            MessageBoxW(main_window, widen(ex.what()).c_str(),
...
struct RiskTextStyle {
    LONG start = 0;
    LONG end = 0;
    COLORREF foreground = RGB(30, 30, 30);
    COLORREF background = RGB(255, 255, 255);
    bool bold = false;
    LONG height_twips = 200;
};

std::pair<COLORREF, COLORREF> risk_colors(const std::string& severity) {
    const auto normalized = point::normalize_name(severity);
    if (normalized == "critical") return {RGB(255, 255, 255), RGB(170, 25, 35)};
    if (normalized == "high") return {RGB(255, 255, 255), RGB(220, 85, 30)};
    if (normalized == "medium" || normalized == "moderate")
        return {RGB(45, 35, 0), RGB(250, 205, 70)};
    if (normalized == "low") return {RGB(15, 70, 35), RGB(190, 235, 200)};
    return {RGB(20, 55, 95), RGB(200, 225, 250)};
}

void apply_risk_report_styles(const std::vector<RiskTextStyle>& styles) {
    if (!risk_report_supports_formatting || !risk_report_text) return;
    SendMessageW(risk_report_text, WM_SETREDRAW, FALSE, 0);
    CHARFORMAT2W base{};
    base.cbSize = sizeof(base);
    base.dwMask = CFM_FACE | CFM_SIZE | CFM_COLOR | CFM_BACKCOLOR;
    base.yHeight = 200;
    base.crTextColor = RGB(35, 42, 50);
    base.crBackColor = RGB(250, 251, 253);
    wcscpy_s(base.szFaceName, LF_FACESIZE, L"Segoe UI");
    SendMessageW(risk_report_text, EM_SETSEL, 0, -1);
    SendMessageW(risk_report_text, EM_SETCHARFORMAT, SCF_SELECTION,
        reinterpret_cast<LPARAM>(&base));
    for (const auto& style : styles) {
        CHARFORMAT2W format{};
        format.cbSize = sizeof(format);
        format.dwMask = CFM_COLOR | CFM_BACKCOLOR | CFM_BOLD | CFM_SIZE;
        format.dwEffects = style.bold ? CFE_BOLD : 0;
        format.crTextColor = style.foreground;
        format.crBackColor = style.background;
        format.yHeight = style.height_twips;
        SendMessageW(risk_report_text, EM_SETSEL,
            static_cast<WPARAM>(style.start),
            static_cast<LPARAM>(style.end));
        SendMessageW(risk_report_text, EM_SETCHARFORMAT, SCF_SELECTION,
            reinterpret_cast<LPARAM>(&format));
    }
    SendMessageW(risk_report_text, EM_SETSEL, 0, 0);
    SendMessageW(risk_report_text, EM_SCROLLCARET, 0, 0);
    SendMessageW(risk_report_text, WM_SETREDRAW, TRUE, 0);
    RedrawWindow(risk_report_text, nullptr, nullptr,
        RDW_INVALIDATE | RDW_ERASE | RDW_UPDATENOW);
}
...
void run_offline_risk_analysis() {
    try {
        ...
        std::wstring report;
        std::vector<RiskTextStyle> styles;
        auto append = [&](const std::wstring& text, COLORREF foreground,
                          COLORREF background, bool bold = false,
                          LONG height_twips = 200) {
            const LONG start = static_cast<LONG>(report.size());
            report += text;
            styles.push_back({start, static_cast<LONG>(report.size()),
                foreground, background, bold, height_twips});
        };
        auto score_text = [](double score) {
            std::wostringstream value;
            value << std::fixed << std::setprecision(1) << score;
            return value.str();
        };
        const auto [summary_text, summary_background] =
            risk_colors(assessment.rating);
        append(L"  OVERALL RISK  •  " + widen(assessment.rating) + L"  \r\n",
            summary_text, summary_background, true, 300);
        append(L"  Entity: " + widen(assessment.entity) + L"\r\n" +
            (assessment.has_authoritative_cvss
                ? L"  Maximum imported CVSS / priority score: "
                : L"  Maximum estimated CVSS-style priority: ") +
            score_text(assessment.maximum_score) + L" / 10.0\r\n" +
            L"  Evidence rows evaluated: " +
            std::to_wstring(evidence.rows.size()) + L"\r\n\r\n",
            RGB(35, 42, 50), RGB(245, 247, 250), false, 210);
        for (std::size_t i = 0; i < assessment.findings.size(); ++i) {
            const auto& finding = assessment.findings[i];
            const auto [finding_text, finding_background] =
                risk_colors(finding.severity);
            append(L"  FINDING " + std::to_wstring(i + 1) + L"  •  " +
                widen(finding.severity) + L"  •  " +
                widen(finding.title) + L"  \r\n",
                finding_text, finding_background, true, 250);
            append(L"  SCORE\r\n  " + score_text(finding.score) +
                (finding.authoritative_cvss
                    ? L" / 10.0 — imported CVSS\r\n"
                    : L" / 10.0 — offline priority estimate\r\n"),
                RGB(45, 55, 70), RGB(238, 242, 247), true);
            append(L"\r\n  EVIDENCE\r\n  " + widen(finding.evidence) + L"\r\n",
                RGB(20, 65, 105), RGB(224, 239, 252), false);
            append(L"\r\n  WHY THIS MATTERS\r\n  " + widen(finding.reason) + L"\r\n",
                RGB(95, 55, 10), RGB(255, 242, 211), false);
            append(L"\r\n  RECOMMENDED ACTION\r\n  " +
                widen(finding.solution) + L"\r\n",
                RGB(20, 80, 45), RGB(225, 246, 231), false);
            std::wstring mappings;
            for (std::size_t m = 0; m < finding.mappings.size(); ++m) {
                if (m) mappings += L"  •  ";
                mappings += widen(finding.mappings[m]);
            }
            append(L"\r\n  FRAMEWORK MAPPINGS\r\n  " + mappings + L"\r\n\r\n",
                RGB(70, 45, 100), RGB(239, 231, 249), false);
        }
        append(L"  LIMITATIONS AND CONFIDENCE  \r\n",
            RGB(255, 255, 255), RGB(90, 98, 108), true, 230);
        std::wstring limitations;
        for (const auto& limitation : assessment.limitations)
            limitations += L"  • " + widen(limitation) + L"\r\n";
        append(limitations, RGB(55, 60, 68), RGB(242, 243, 245), false);
        SetWindowTextW(risk_report_text, report.c_str());
        apply_risk_report_styles(styles);
...
LRESULT CALLBACK window_proc(HWND window, UINT message,
    ...
    case WM_CREATE: {
        ...
        AppendMenuW(
            workspace_menu, MF_STRING,
            ID_MENU_COMPRESS_FILE, L"Compress File... (Fast)");
        AppendMenuW(
            workspace_menu, MF_STRING,
            ID_MENU_COMPRESS_FILE_BALANCED, L"Compress File... (Balanced)");
        AppendMenuW(
            workspace_menu, MF_STRING,
            ID_MENU_DECOMPRESS_FILE, L"Decompress File...");
        AppendMenuW(workspace_menu, MF_SEPARATOR, 0, nullptr);
        AppendMenuW(
            workspace_menu, MF_STRING,
        ...
        input_drop_zone = CreateWindowW(L"STATIC",
            ...
            L"Supported: .csv, .xlsx, .xls (.xlsm is blocked)",
        ...
        if (GetSystemDirectoryW(system_directory, MAX_PATH) != 0) {
            std::filesystem::path rich_edit_path(system_directory);
            rich_edit_path /= L"Msftedit.dll";
            risk_rich_edit_module = LoadLibraryW(rich_edit_path.c_str());
        }
        risk_report_supports_formatting = risk_rich_edit_module != nullptr;
        const wchar_t* risk_report_class = risk_report_supports_formatting
            ? MSFTEDIT_CLASS : L"EDIT";
        risk_report_text = CreateWindowExW(
            WS_EX_CLIENTEDGE, risk_report_class, nullptr,
            WS_CHILD | WS_VSCROLL | ES_MULTILINE |
                ES_AUTOVSCROLL | ES_READONLY,
            24, 158, 900, 400, window,
            reinterpret_cast<HMENU>(static_cast<INT_PTR>(ID_RISK_REPORT)),
            nullptr, nullptr);
        if (risk_report_supports_formatting) {
            SendMessageW(risk_report_text, EM_SETBKGNDCOLOR, 0,
                static_cast<LPARAM>(RGB(250, 251, 253)));
        } else {
            SendMessageW(risk_report_text, WM_SETFONT,
                reinterpret_cast<WPARAM>(GetStockObject(DEFAULT_GUI_FONT)), TRUE);
        }
        SetTimer(window, AUTO_SCHEDULE_TIMER_ID, 30'000, nullptr);
        SetTimer(window, INBOX_EVENT_POLL_TIMER_ID, 1'000, nullptr);
    ...
    case WM_COMMAND: {
        ...
        if (notification == EN_SETFOCUS && !internal_cell_update &&
            id >= ID_CELL_BASE &&
            id < ID_CELL_BASE + GRID_CONTROL_ROWS * GRID_COLUMNS) {
            const int visible_index = id - ID_CELL_BASE;
            const int logical_row =
            ...
            focused_edit_key = cell_key(logical_row, column);
            focused_edit_original = stored_cell(logical_row, column);
        }
        ...
        case ID_SCHEDULE_FETCHER:
            ShellExecuteW(main_window, L"open",
                (application_install_root / "PointFetcher.exe").c_str(),
                nullptr, application_install_root.c_str(), SW_SHOWNORMAL);
            return 0;
        ...
        case ID_MENU_COMPRESS_FILE:
            start_file_compression(
                false, point::compression::Profile::Fast); return 0;
        case ID_MENU_COMPRESS_FILE_BALANCED:
            start_file_compression(
                false, point::compression::Profile::Balanced); return 0;
        case ID_MENU_DECOMPRESS_FILE:
            start_file_compression(true); return 0;
    ...
    case WM_POINT_COMPRESSION_COMPLETE: {
        std::unique_ptr<CompressionCompletion> completion(
            reinterpret_cast<CompressionCompletion*>(lparam));
        compression_running.store(false);
        if (compression_thread.joinable()) compression_thread.join();
        if (!completion) return 0;
        if (!completion->succeeded) {
            set_status(L"Point Fast Compression failed.");
            MessageBoxW(main_window, completion->error.c_str(),
                L"Point Fast Compression", MB_ICONERROR);
            point::append_audit(app_root,
                completion->decompress ? "FILE_DECOMPRESSION_FAILED" :
                    "FILE_COMPRESSION_FAILED", "operation failed");
            return 0;
        }
        std::wstringstream summary;
        summary << (completion->decompress ? L"Decompression" : L"Compression")
                << L" completed.\r\n\r\nOutput: "
                << completion->output.wstring() << L"\r\nInput bytes: "
                << completion->result.input_bytes << L"\r\nOutput bytes: "
                << completion->result.output_bytes << L"\r\nCompressed blocks: "
                << completion->result.compressed_blocks << L"\r\nRaw blocks: "
                << completion->result.raw_blocks;
        if (!completion->decompress)
            summary << L"\r\nMode: " <<
                (completion->profile == point::compression::Profile::Balanced
                    ? L"Balanced" : L"Fast");
        set_status(completion->decompress
            ? L"File decompressed successfully."
            : completion->profile == point::compression::Profile::Balanced
                ? L"File compressed successfully with Point Balanced Mode."
                : L"File compressed successfully with Point Fast Mode.");
        MessageBoxW(main_window, summary.str().c_str(),
            L"Point Fast Compression", MB_ICONINFORMATION);
        point::append_audit(app_root,
            completion->decompress ? "FILE_DECOMPRESSED" : "FILE_COMPRESSED",
            "completed; output_bytes=" +
                std::to_string(completion->result.output_bytes));
        return 0;
    }
    case WM_DRAWITEM: {
    ...
    case WM_TIMER:
        if (wparam == INBOX_EVENT_POLL_TIMER_ID) {
            if (inbox_changed_event &&
                WaitForSingleObject(inbox_changed_event, 0) == WAIT_OBJECT_0) {
                ResetEvent(inbox_changed_event);
                if (refresh_running.load()) {
                    SetTimer(window, INBOX_NOTIFICATION_TIMER_ID, 1'000, nullptr);
                    set_status(L"A validated download is waiting; Imported Files will refresh automatically.");
                } else {
                    refresh_engine(true);
                }
            }
    ...
    case WM_DESTROY:
        ...
        if (inbox_changed_event) {
            CloseHandle(inbox_changed_event);
            inbox_changed_event = nullptr;
        }
        PostQuitMessage(0);
        return 0;
    }
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int show) {
    ...
    try {
        application_install_root = point::install_root();
        app_root = point::user_data_root();
        point::migrate_legacy_data(application_install_root, app_root);
        point::ensure_directories(app_root);
        compliance_policy =
            point::compliance::load_policy(application_install_root);
    ...
    InitCommonControlsEx(&controls);
    inbox_changed_event = point::create_inbox_changed_event();
    if (!inbox_changed_event) {
        MessageBoxW(nullptr,
            L"Point could not create its protected local notification event.",
            L"Point startup failed", MB_ICONERROR);
        return 1;
    }
