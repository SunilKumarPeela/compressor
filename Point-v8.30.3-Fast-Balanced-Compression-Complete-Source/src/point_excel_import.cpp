...
bool run_native_xlsx_reader(const std::filesystem::path& script,
    ...
    std::error_code powershell_error;
    if (!std::filesystem::is_regular_file(powershell, powershell_error) ||
        powershell_error)
        return false;
    std::wstring command =
        command_argument(powershell.wstring()) +
        ...
        L" -CacheKey " + command_argument(cache_key) +
    ...
    if (!CreateProcessW(powershell.c_str(), mutable_command.data(),
                        nullptr, nullptr, FALSE,
                        CREATE_NO_WINDOW, nullptr, nullptr, &startup, &process))
        return false;
    // A malformed or unusually complex workbook must not block Refresh
    // forever. Large imports retain a generous bounded execution window.
    const DWORD wait_status =
        WaitForSingleObject(process.hProcess, 15U * 60U * 1000U);
...
std::vector<std::filesystem::path> generated_sheets(
        const std::filesystem::path& cache, const std::wstring& cache_key,
        const std::wstring& stem) {
    const std::wstring prefix = cache_key + L"__" + stem + L"__";
...
std::wstring workbook_cache_key(const std::filesystem::path& workbook) {
    // Stable FNV-1a identifier avoids positional cache invalidation when a
    // workbook is added or removed. Only the filename is used because Inbox
    // rejects duplicate destination names.
    std::uint64_t hash = 14'695'981'039'346'656'037ull;
    auto name = workbook.filename().wstring();
    ...
    output << L"wb" << std::hex << std::setw(16) << std::setfill(L'0') << hash;
    return output.str();
}

std::string workbook_cache_signature(
        const std::filesystem::path& workbook) {
    std::error_code error;
    const auto size = std::filesystem::file_size(workbook, error);
    if (error) return {};
    const auto modified = std::filesystem::last_write_time(workbook, error);
    if (error) return {};
...
void write_workbook_cache_manifest(
        const std::filesystem::path& manifest,
        const std::string& signature) {
    std::ofstream output(manifest, std::ios::binary | std::ios::trunc);
    if (!output) throw std::runtime_error("Workbook cache manifest write failed");
    output << signature;
}

void clear_workbook_cache(
        const std::filesystem::path& cache,
        const std::wstring& cache_key,
        const std::wstring& stem) {
    const auto prefix = cache_key + L"__" + stem + L"__";
    for (const auto& entry : std::filesystem::directory_iterator(cache)) {
        std::error_code error;
...
ExcelImportResult prepare_import_sources(
    ...
    std::set<std::size_t> reused_imports;
    for (std::size_t index = 0; index < workbooks_to_import.size(); ++index) {
        const auto& workbook = workbooks_to_import[index];
        const auto key = workbook_cache_key(workbook);
        const auto stem = safe_component(workbook.stem().wstring());
        const auto manifest = cache / (key + L"__" + stem + L".manifest");
        const auto signature = workbook_cache_signature(workbook);
        auto sheets = generated_sheets(cache, key, stem);
        if (!workbook_cache_matches(manifest, signature) || sheets.empty()) {
            clear_workbook_cache(cache, key, stem);
            continue;
        }
        reused_imports.insert(index + 1);
        result.cache_reused = true;
        ++result.workbook_count;
        result.worksheet_count += sheets.size();
        result.csv_sources.insert(
            result.csv_sources.end(), sheets.begin(), sheets.end());
        if (progress)
            progress(index + 1, workbooks_to_import.size(), workbook, true);
    }
    ...
    const auto native_script = point::install_root() / L"scripts" /
                               L"point_xlsx_to_csv.ps1";
    for (std::size_t index = 0; index < workbooks_to_import.size(); ++index) {
        const auto& workbook_path = workbooks_to_import[index];
        if (reused_imports.contains(index + 1)) continue;
        ...
        const auto key = workbook_cache_key(workbook_path);
        if (!run_native_xlsx_reader(native_script, workbook_path, cache,
                                    key, stem))
            continue;
        auto sheets = generated_sheets(cache, key, stem);
        if (sheets.empty()) continue;
        write_workbook_cache_manifest(
            cache / (key + L"__" + stem + L".manifest"),
            workbook_cache_signature(workbook_path));
    ...
    if (native_imports.size() + reused_imports.size() ==
        workbooks_to_import.size()) {
        return result;
    }
    ...
    try {
        ...
        for (const auto& workbook_path : workbooks_to_import) {
            ...
            ++workbook_ordinal;
            if (native_imports.count(workbook_ordinal) != 0 ||
                reused_imports.count(workbook_ordinal) != 0) continue;
            if (progress)
                progress(workbook_ordinal, workbooks_to_import.size(),
                         workbook_path, false);
            const auto safe_stem = safe_component(workbook_path.stem().wstring());
            const auto cache_key = workbook_cache_key(workbook_path);
            ...
            try {
                ...
                write_workbook_cache_manifest(
                    cache / (cache_key + L"__" + safe_stem + L".manifest"),
                    workbook_cache_signature(workbook_path));
                std::error_code cleanup_error;
