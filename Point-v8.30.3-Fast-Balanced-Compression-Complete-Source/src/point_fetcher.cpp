...
bool supported_name(const std::wstring& name) {
    if (name.empty() || name.size() > 240 ||
        std::filesystem::path(name).filename().wstring() != name ||
        name == L"." || name == L".." ||
...
void add_schedule() {
    ...
    if (schedule.filename.empty() ||
        ...
        throw std::runtime_error("Enter a safe .csv, .xlsx, or .xls filename");
    ...
    case WM_FETCH_COMPLETE: {
        ...
        if (result && result->index < schedules.size()) {
            ...
            if (result->browser_fallback) {
                const auto browser = installation_root / L"PointBrowserFetcher.exe";
                ...
                const auto launched = reinterpret_cast<INT_PTR>(ShellExecuteW(
                    ...
                    installation_root.c_str(), SW_SHOWNORMAL));
                if (launched > 32) {
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int show) {
    installation_root = point::install_root();
    const auto app_root = point::user_data_root();
    point::migrate_legacy_data(installation_root, app_root);
