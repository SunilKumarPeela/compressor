...
bool allowed_extension(const std::filesystem::path& path) {
    ...
    return ext == L".csv" || ext == L".xlsx" || ext == L".xls";
}

bool certificate_error(COREWEBVIEW2_WEB_ERROR_STATUS status) {
    return status == COREWEBVIEW2_WEB_ERROR_STATUS_CERTIFICATE_COMMON_NAME_IS_INCORRECT ||
                                ...
                                Callback<ICoreWebView2NavigationCompletedEventHandler>(
                                    [](ICoreWebView2*, ICoreWebView2NavigationCompletedEventArgs* args) -> HRESULT {
                                        ...
                                        if (!success) {
                                            ...
                                            if (certificate_error(status)) {
                                                SetWindowTextW(status_label,
                                                    L"Blocked: the website certificate is invalid, expired, revoked, or issued for another name.");
                                            }
                                        }
                                        return S_OK;
                                    }).Get(), &completed_token);
                            ...
                            webview->add_NewWindowRequested(
                                Callback<ICoreWebView2NewWindowRequestedEventHandler>(
                                    [](ICoreWebView2*, ICoreWebView2NewWindowRequestedEventArgs* args) -> HRESULT {
                                        ...
                                        SetWindowTextW(status_label,
                                            L"Blocked pop-up window. Open an approved HTTPS address in the main address box.");
                                        return S_OK;
                                    }).Get(), &new_window_token);
