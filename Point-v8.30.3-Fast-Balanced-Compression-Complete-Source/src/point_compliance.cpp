...
void protect_file_for_current_user(const std::filesystem::path& path) {
    ...
    auto packed = point::compression::encode_fast(plain);
    DATA_BLOB input{
        static_cast<DWORD>(packed.size()),
        packed.data()};
    DATA_BLOB output{};
    if (!CryptProtectData(
...
std::string read_user_protected_file(const std::filesystem::path& path) {
    ...
    std::string result;
    if (point::compression::is_fast_envelope(
            std::span<const std::uint8_t>(
                reinterpret_cast<const std::uint8_t*>(output.pbData),
                output.cbData))) {
        const auto unpacked = point::compression::decode_fast(
            std::span<const std::uint8_t>(
                reinterpret_cast<const std::uint8_t*>(output.pbData),
                output.cbData));
        result.assign(
            reinterpret_cast<const char*>(unpacked.data()),
            unpacked.size());
    } else {
        // Backward compatibility for DPAPI-protected v8 workspace files that
        // predate Point Fast Compression.
        result.assign(
