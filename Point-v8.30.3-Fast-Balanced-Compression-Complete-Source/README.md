# Point v8.30.3 — Fast and Balanced Compression

Adds a user-selectable **Balanced** compression profile. Fast mode keeps the
single-candidate low-latency matcher; Balanced mode searches eight recent
candidates per hash and indexes positions inside matches to find longer and
more useful repetitions. Both profiles produce compatible `.pfc` files, and
Decompress detects the selected method automatically. Balanced uses more CPU
and memory in exchange for potentially smaller output; already-compressed or
high-entropy input may still receive raw-block fallback.

# Point v8.30.2 — Fast Compression

Adds Point Fast Compression for DPAPI-protected workspace state and new
**Workspace > Compress File... (Fast Mode)** and **Decompress File...**
commands for arbitrary files. The `.pfc` container processes independent
1 MiB blocks, samples incompressible input for immediate raw fallback, checks
every decoded block with CRC32, enforces bounded output sizes, publishes output
atomically, and performs file operations on a background thread. This release
also centralizes version metadata, corrects per-user installer shortcuts, and
adds release-consistency, regression, DPAPI-integration, artifact, and static
analysis checks to CI.

# Point v8.30.1 — Fetcher Link Correction

Adds the Windows `ole32.lib` dependency required by Point Fetcher's
`%LOCALAPPDATA%` path resolver. This corrects the `LNK2019` unresolved
`CoTaskMemFree` build failure without changing runtime behavior.

# Point v8.30.0 — Enterprise Boundary Hardening

- Replaces global window-message broadcasts with a per-user named event and a
  protected Windows ACL.
- Blocks macro-enabled XLSM files at selection, import, validation, Fetcher,
  and Browser Fetcher boundaries.
- Blocks WebView2 pop-ups, reports certificate failures explicitly, and keeps
  HTTPS-only navigation and downloads.
- Separates installed binaries from user data. Inbox, Workspace, Exports,
  Logs, Fetcher state, staging, and the browser profile now reside under
  `%LOCALAPPDATA%\Point`.
- Migrates existing Point data non-destructively on first launch.

# Point v8.29.1 — Security-Reviewed Complete Source

This maintenance release pins the Excel-reader PowerShell executable to the
Windows System32 path and rejects unsafe Fetcher filenames both at entry and
when saved configuration is reloaded. See `SECURITY-REVIEW.md` for the reviewed
trust boundaries, protections, remaining limitations, and validation scope.

# Point v8.29.0 — Visual Risk Cards and Fast Refresh

Risk Analysis now presents offline evidence in color-coded blocks for severity,
evidence, impact, remediation, framework mappings, and limitations. Refresh now
reuses the validated relationship graph and on-demand lookup state when source
reports are unchanged, while changed reports still receive full validation.

# Point v8.28.0 — Bounded Multi-Level Undo

Ctrl+Z now repeatedly undoes completed cell edits, pasted ranges, and cleared
selections, with each user action treated as one history step. Ctrl+U has a
separate multi-level history for learned text transformations and duplicate-row
removals. Histories retain up to 100 cell actions and 20 workspace operations,
share strict 64 MiB memory budgets, discard oldest entries first, restore exact
row positions, and audit counts without recording cell values.

# Point v8.27.1 — Fast Deferred Paste Validation

Large clipboard operations now write values to the sparse workspace and repaint
once instead of performing a complete cross-report identity scan for every
pasted cell. Only populated cells in trusted identity target columns are added
to the deduplicated validation queue, and secure name resolution occurs when
Search is requested. Ordinary columns bypass identity preparation entirely;
ambiguous names remain blocked rather than guessed.

# Point v8.27.0 — Incremental Workbook Refresh

Excel conversion caches are now independent and stable per workbook. Adding,
removing, or replacing one report processes only that report; unchanged
workbooks reuse their validated worksheet caches and existing dataset indexes.
Cache identity no longer depends on file-list position, so inserting a new
workbook cannot force unrelated reports to be reconverted. Modified reports
still receive complete sheet, header, date, relationship, and index validation.
...
## Point Fast Compression

Saved workspace views use Point Fast Compression before the existing Windows
DPAPI protection step. Fast mode is deliberately bounded and dependency-free:

- a small sample rejects random, encrypted, or already-compressed input early;
- one linear LZ candidate is evaluated instead of competing across every
  reversible transform;
- output automatically falls back to raw bytes unless compression saves at
  least 1% and covers its metadata cost;
- the PFC1 envelope records the original and payload sizes and a CRC32;
- decoding validates all lengths, references, the 64 MiB output limit, and the
  checksum before the workspace parser receives any data;
- older DPAPI-protected v8 workspace views remain readable.

Compression occurs only when a workspace is saved or loaded. It does not run
in Point's search, indexing, spreadsheet import, grid, Count, Compare, Chart,
or Risk Analysis hot paths.

Point also exposes separate **Workspace > Compress File... (Fast)**,
**Compress File... (Balanced)**, and **Decompress File...** commands. The
compressor accepts any file and writes
a `.pfc` archive. The decompressor validates and restores the exact original
bytes to a user-selected path. File operations run on a background thread and
use independent 1 MiB blocks, bounded memory, CRC32 checks, a 2 GiB safety
limit, and atomic output publication. Point never deletes or modifies the
...
Point v8.30.3 accepts `.xlsx` and `.xls` when Microsoft Excel is
...
Macro-enabled `.xlsm` files are intentionally blocked at every input boundary.
